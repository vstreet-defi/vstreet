...
const InventoryManager: React.FC = () => {
    ...
    const handleUnlockPosition = async (positionId: bigint) => {
        ...
        try {
            ...
            const transaction = sailsVault.services.VaultService.functions.UnlockAndClaimPosition(
                positionId
            );
    ...
    const handleClaimAll = async () => {
        if (!accountData || accounts.length === 0 || maturedPositions.length === 0) {
        ...
        try {
            ...
            const transaction = sailsVault.services.VaultService.functions.ClaimMultiplePositions(
                positionIds
            );
            transaction.withAccount(accountData.address, {
                signer: signer as string | CodecClass<Codec, any[]> as Signer,
                ...
                return {
                    items: allPositions
                        ...
                        .map((position: VaultPosition) => ({
                            ...
                            secondaryValue: `${formatAmount(position.amount)} $VST (${getMultiplierString(position.conviction_level)})`,
                            tertiaryLabel: "Date",
