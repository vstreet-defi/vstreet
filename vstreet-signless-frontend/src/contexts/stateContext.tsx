...
const LiquidityContext = createContext<LiquidityContextProps | undefined>(undefined);

interface LiquidityProviderProps {
  children: React.ReactNode;
}

export const LiquidityProvider: React.FC<LiquidityProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const getLiquidityData = async () => {
      try {
        ...
        // In sails-js, you need to query from an account (here we use a contract owner, e.g., bob)
        const bob = '0xfe0a346d8e240f29ff67679b83506e92542d41d87b2a6f947c4261e58881a167';

        // --- MAIN FIX: Use Service (capital "S") ---
        if (
          !sails.services?.Service ||
          !sails.services.Service.queries?.ContractInfo
        ) {
          console.error('[LiquidityProvider] Service/ContractInfo query not found in IDL!', sails.services);
          return;
        }
        ...
        console.error('[LiquidityProvider] Error getting liquidity data:', err);
      }
    };
  ...
  return <LiquidityContext.Provider value={{ liquidityData }}>{children}</LiquidityContext.Provider>;
};
