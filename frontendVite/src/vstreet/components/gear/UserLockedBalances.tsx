...
function UserLockedBalances() {
  ...
  // Add your programID
  const programIDFT = '0xe996768dde1010e993d21871214fd6a420063d9dba0ad7a1b6ddc8dcfe56a0de';
  ...
  const getCollateralLocked = () => {
    const findAccount = (address: string) => lenders.find((array: string | any[]) => array.includes(address));

    const userData = findAccount(walletDecoded);

    // Check if userData is defined and has the expected structure
    if (userData && userData.length > 1 && userData[1] && userData[1].liquidity !== undefined) {
  ...
  const getSyntheticlLocked = () => {
    const findAccount = (address: string) => borrowers.find((array: string | any[]) => array.includes(address));

    const userData = findAccount(walletDecoded);

    // Check if userData is defined and has the expected structure
    if (userData && userData.length > 1 && userData[1] && userData[1].loanamount !== undefined) {
  ...
  return (
    <Box className={styles.Moduleborderwrap}>
      <Box className={styles.modulePositions}>
        <Heading bgGradient="linear(to-l, #4FFF4B, #00FFC4 )" bgClip="text" fontSize="2rem" mb="3rem">
          My Positions
        </Heading>
