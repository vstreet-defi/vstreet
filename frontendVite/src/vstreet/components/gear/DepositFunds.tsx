...
function DepositFunds() {
  ...
  //USDCV
  // Add your programID
  const programIDFT = '0xd8d0206ab4a4d0f26f80a28594e767d62bf1d5ff436c8c79e819a97399b7eaa7';
  ...
  const signer = async () => {
    if (!api || !account) {
      alert.error('API or account not available');
      return;
    }

    const accounts = await web3Accounts();
    const isVisibleAccount = accounts.some((visibleAccount: any) => visibleAccount.address === account.address);
  ...
  return (
    <Box className={styles.Moduleborderwrap}>
      <Box className={styles.module}>
        ...
        <Flex mt="1rem">
          <Input color="white" value={valueAmount} onChange={AmountInputChange} />
