    ...
    <div className={styles.myPositionCard}>
      ...
      <div className={styles.stats}>
        <div className={styles.statItem}>
          ...
          <p className={styles.statValue}>${formatWithCommasVARA(userInfo?.balance_vara ?? 0)} TVARA</p>
        </div>

        <div className={styles.statItem}>
          <p className={styles.statLabel}>Available To Borrow</p>
          <p className={styles.statValue}>${formatWithCommasVUSD(userInfo?.mla ?? 0)} wUSD</p>
        </div>

        <div className={styles.statItem}>
          <p className={styles.statLabel}>Current Loan/Debt</p>
          <p className={styles.statValue}>
            ${userInfo?.loan_amount ? formatWithCommasVUSD(userInfo.loan_amount) : 0} wUSD
          </p>
        </div>

        <div className={styles.statItem}>
          <p className={styles.statLabel}>Loan To Value (LTV)</p>
          <p className={`${styles.statValue} ${ltv > 70 ? styles.danger : ''}`}>{ltv}%</p>
        </div>

        <div className={styles.statItem}>
          <p className={styles.statLabel}>Daily Loan Interest</p>
          <p className={styles.statValue}>{formatDailyInterest(liquidityData?.liquidityData?.InterestRate ?? 0)}%</p>
        </div>
      </div>
    </div>
  );
